...
def test_TriageResult(requests_mock):
    ...
    BEHAVIORAL1 = os.path.join(
        os.path.dirname(__file__), "triage_responses", "240202-3y8f7sefen-behavioral1-report_triage.json"
    )
    BEHAVIORAL2 = os.path.join(
        os.path.dirname(__file__), "triage_responses", "240202-3y8f7sefen-behavioral2-report_triage.json"
    )
    ...
    requests_mock.get(
        "https://api.tria.ge/v0/samples/240202-3y8f7sefen/behavioral1/report_triage.json", text=behavioral1_data
    )
    requests_mock.get(
        "https://api.tria.ge/v0/samples/240202-3y8f7sefen/behavioral2/report_triage.json", text=behavioral2_data
    )
